...
File: scripts\sync-fnlla-runtime.ps1
...
param(
    [string]$SourcePath,
    [string]$RepoUrl,
    [string]$Repository = "techayoDEV/fnlla",
...
function Try-CloneSource {
    ...
    try {
        ...
        $attempts += @{
            Label = "git"
            FilePath = $gitPath
            Arguments = @("clone", "--depth", "1", "git@github.com:techayoDEV/fnlla.git", $ClonePath)
        }
        $attempts += @{
            Label = "git"
            FilePath = $gitPath
            Arguments = @("clone", "--depth", "1", "https://github.com/techayoDEV/fnlla.git", $ClonePath)
        }
        ...
        throw "Unable to clone maintained repository $ProjectRepository. Attempts failed: $($errors -join ' | ')"
    }
    finally {
...
try {
    if ($SourcePath) {
        ...
        if ($WorkingClonePath) {
            ...
            $cloneRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("fnlla-runtime-sync-" + [System.Guid]::NewGuid().ToString("N"))
