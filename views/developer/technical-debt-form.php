...
<form class="debt-form" method="post" action="<?= h(route("developer.panel.technical_debt.save")) ?>">
  ...
  <label>Status<select name="status"><?php foreach (TechnicalDebtRegistry::STATUSES as $status): ?><option value="<?= h($status) ?>" <?= $item["status"] === $status ? "selected" : "" ?>><?= h(ucwords(str_replace("_", " ", $status))) ?></option><?php endforeach; ?></select></label>
  <label>Priority<select name="priority"><?php foreach (TechnicalDebtRegistry::PRIORITIES as $priority): ?><option value="<?= h($priority) ?>" <?= $item["priority"] === $priority ? "selected" : "" ?>><?= h(ucfirst($priority)) ?></option><?php endforeach; ?></select></label>
