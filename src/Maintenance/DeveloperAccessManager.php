...
final class DeveloperAccessManager
{
    ...
    public function unlock(Request $request, string $password): array
    {
        if (!$this->enabled() || !$this->configured()) {
            return [
                "success" => false,
        ...
        if (!$this->safeEquals($this->configuredPassword(), trim($password))) {
            ...
            return [
                "success" => false,
                "error" => "Incorrect developer access password. Please try again.",
    ...
    {
        return "developer-access:" . sha1($request->ip());
    }
